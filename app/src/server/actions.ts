...
export const stripePayment: StripePayment<string, StripePaymentResult> = async (
  ...
  let priceId
  if (tier === TierIds.ANNUAL) {
    priceId = process.env.ANNUAL_ACCESS_PRICE_ID!
  } else if (tier === TierIds.LIFETIME) {
    priceId = process.env.LIFETIME_ACCESS_PRICE_ID!
  } else {
  ...
  try {
    ...
    session = await createStripeCheckoutSession({
      ...
      mode: tier === TierIds.LIFETIME ? 'payment' : 'subscription',
    })
    if (!session) {
...
export const createFile: CreateFile<fileArgs, File> = async (
  { fileType, name },
  context,
) => {
  if (!context.user) {
